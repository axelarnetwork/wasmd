...
// buggyCallDepthMessageHandler is a wrapper around a Messenger that checks the call depth before dispatching a message.
type buggyCallDepthMessageHandler struct {
	Messenger
	MaxCallDepth uint32
}

// TODO: remove during next coordinated upgrade
func (h buggyCallDepthMessageHandler) DispatchMsg(ctx sdk.Context, contractAddr sdk.AccAddress, contractIBCPortID string, msg wasmvmtypes.CosmosMsg) (events []sdk.Event, data [][]byte, err error) {
	ctx, err = checkAndIncreaseCallDepthBuggy(ctx, h.MaxCallDepth)
	if err != nil {
		return nil, nil, err
	}
